...
class MagentoXmlrpcClient extends Client
{
    ...
    public function __destruct()
    {
        if ($this->autoCloseSession && $this->client) {
            $this->client->call('endSession', array($this->configCollection->get('session')));
        }
    }
    ...
    public function getClient()
    {
        if (!isset($this->client)) {
            ...
            $this->client = new \fXmlRpc\Client(
                $uri,
                new \fXmlRpc\Transport\HttpAdapterTransport(new \Ivory\HttpAdapter\GuzzleHttpHttpAdapter($this))
            );
        }
